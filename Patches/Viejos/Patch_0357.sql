...
/****** Object:  View [dbo].[View_Contratos]    Script Date: 04/29/2014 14:35:04 ******/
SET ANSI_NULLS ON
GO
...
(SELECT     *
FROM          Direcciones
WHERE      idContrato = con.idContrato) THEN
(SELECT     idDirección
FROM          dbo.Direcciones
WHERE      idContrato = con.idContrato) ELSE dir.idDireccion END AS Expr1, cli.RazonSocial + CASE WHEN ISNULL(cli.NombreFantasia, '') 
...
(SELECT     idDirección
FROM          dbo.Direcciones
WHERE      idContrato = con.idContrato) ELSE dir.idDireccion END) AS DireccionCliente
FROM         dbo.Clientes AS cli LEFT OUTER JOIN
dbo.Contratos AS con ON con.idCliente = cli.idCliente LEFT OUTER JOIN
dbo.Direcciones AS dir ON dir.idCliente = cli.idCliente
WHERE     (dir.Real = 1)
