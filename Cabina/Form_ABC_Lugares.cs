...
namespace Cabina
{
    ...
    public partial class FormABCLugares : Form1
    {
            ...
            var fc = lugaresUltraGrid.ActiveRow.Cells["idLocalidad"].Value == DBNull.Value
                         ...
                         : new FilterCondition(e.Row.Band.Columns["idLocalidad"], FilterComparisionOperator.Equals,
                                               dsLocalidadesLista.LocalidadesLista.FindByidLocalidad(
                                                   (int) lugaresUltraGrid.ActiveRow.Cells["idLocalidad"].Value).
                                                   idLocalidad);
        ...
        private void LugaresUltraGridBeforeRowsDeleted(object sender, BeforeRowsDeletedEventArgs e)
        {
            ...
            MessageBox.Show("Tiene hijos.", "¡No se puede borrar!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            e.Cancel = true;
        }
        ...
        private bool Verificado()
        {
            ...
            foreach (UltraGridRow lugar in lugares.GetRowEnumerator(GridRowType.DataRow))
            {
                foreach (var direccion in lugar.ChildBands["FK_Direcciones_Lugares"].Rows)
                {
                    ...
                    {
                        MessageBox.Show(
                            "La calle " + direccion.Cells["idCalle"].Text + " no pertenece a " +
                            direccion.Cells["idLocalidad"].Text + ".", "No se pudo grabar", MessageBoxButtons.OK,
                            MessageBoxIcon.Error);
                        return false;
                    }
                    if (direccion.Cells["idEntreCalle1"].Value != DBNull.Value &&
                        dsCallesLista1.Calles.FindByidCalle((int) direccion.Cells["idEntreCalle1"].Value).
                            idLocalidad != (int) direccion.Cells["idLocalidad"].Value)
                    {
                        MessageBox.Show(
                            "La calle " + direccion.Cells["idEntreCalle1"].Text + " no pertenece a " +
                            direccion.Cells["idLocalidad"].Text + ".", "No se pudo grabar", MessageBoxButtons.OK,
                            MessageBoxIcon.Error);
                        return false;
                    }
                    if (direccion.Cells["idEntreCalle2"].Value != DBNull.Value &&
                        dsCallesLista1.Calles.FindByidCalle((int) direccion.Cells["idEntreCalle2"].Value).
                            idLocalidad != (int) direccion.Cells["idLocalidad"].Value)
                    {
                        MessageBox.Show(
                            "La calle " + direccion.Cells["idEntreCalle2"].Text + " no pertenece a " +
                            direccion.Cells["idLocalidad"].Text + ".", "No se pudo grabar", MessageBoxButtons.OK,
                            MessageBoxIcon.Error);
                        return false;
                    }
                    ...
                    MessageBox.Show(
                        "El barrio " + direccion.Cells["idBarrio"].Text + " no pertenece a " +
                        direccion.Cells["idLocalidad"].Text + ".", "No se pudo grabar", MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    return false;
                }
