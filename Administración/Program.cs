...
namespace Administración
{
    ...
    internal static class ManejoDatos
    {
        ...
        public static string FechaConBarras(string yyyymmdd)
        {
            return yyyymmdd.Substring(6, 2) + @"/" + yyyymmdd.Substring(4, 2) + @"/" + yyyymmdd.Substring(0, 4);
        }

        public static string CodePage(string utf8String)
        {
        ...
        public static void SeleccionInicio(object ugrid)
        {
            if (((UltraGrid)ugrid).ActiveCell.Value.GetType().ToString() == "System.Boolean" 
                //||
                //((UltraGrid)ugrid).ActiveCell.Value.GetType().ToString() == "System.DBNull"
                )
                return;
